 ...
 *  so wins and losses always alternate (no two LOST ever touch). */
function buildSlots(seed: number): Slot[] {
  ...
  const losts = tokens.map((_, i) => i); // dummy values, equal count
...
export default function LuckyWheelGame() {
  ...
  // Pre-spin shuffle is local-random (just a preview). After a spin confirms,
  // we re-derive the seed from the tx hash + result bytes so the layout
  // shown is provably tied to the on-chain randomness.
  ...
  const play = async () => {
    ...
    try {
      const r = await casino.playLuckyWheel(pick, finalBet);

      // Re-seed the layout from on-chain randomness so the displayed positions
      // are derived from the same entropy that decided the outcome.
      ...
      // Apply new layout BEFORE animation so the wheel spins to the on-chain slot
      setShuffleSeed(onChainSeed);
      setAngle(prev => prev + (target - (prev % 360)));
      setTx({ hash: r.txHash, status: 'confirmed' });
      history.add({ game: 'LuckyWheel', bet: finalBet, payout: r.payout, win: r.win, txHash: r.txHash });

      setTimeout(() => {
        const landed = newSlots[byte];
        const landedToken = landed?.kind === 'win' ? landed.token : undefined;
        // Use realized (chain) payout for the label when it's a win, so the
        // amount shown is exactly what the contract paid out — never hardcoded.
        ...
        notifyResult({
          game: 'Lucky Wheel', win: r.win, payout: r.payout,
          rewardLabel, txHash: r.txHash, txStatus: 'confirmed',
        });
        if (r.win && landedToken) {
          setConfetti(c => c + 1);
          // Track the auto-swap tx so the UI can prove the user really
          // received the reward token on-chain (not just zkLTC).
      ...
      // Mark failed — notifyResult is gated on confirmed status, so neither
      // a win/lose toast nor a sound will fire here. Only the friendly error.
      setTx(t => ({ ...t, status: 'failed' }));
      toast.error(friendlyError(e));
    }
  };

  // Alternating accent ring; LOST slots always render dark regardless of index.
  const accentColors = [
    'oklch(0.65 0.25 330)', 'oklch(0.78 0.16 85)',
    'oklch(0.55 0.20 200)', 'oklch(0.60 0.22 150)',
    'oklch(0.55 0.25 25)',  'oklch(0.65 0.20 280)',
    'oklch(0.70 0.18 50)',  'oklch(0.50 0.22 320)',
  ];

  return (
    <GameShell title="Lucky Wheel" subtitle={`${SLOTS.filter(s=>s.kind==='win').length} token rewards + ${SLOTS.filter(s=>s.kind==='lost').length} LOST — layout reseeded from on-chain randomness`} icon="🎡" accent="gold">
      <Confetti trigger={confetti} />
      <div className="casino-felt rounded-2xl p-6 flex items-center justify-center min-h-[340px] relative">
        <div className="relative" style={{ width: SLOTS.length > 8 ? 340 : 300, height: SLOTS.length > 8 ? 340 : 300 }}>
          <div className="absolute left-1/2 -top-2 -translate-x-1/2 z-20 pointer-pulse">
            <svg width="28" height="34" viewBox="0 0 20 24"><path d="M10 24 L0 0 L20 0 Z" fill="oklch(0.95 0.15 90)" /></svg>
          </div>
          <div
            className="roulette-wheel absolute inset-0 rounded-full border-4 border-wolf-gold overflow-hidden transition-transform duration-[4000ms] ease-out"
            style={{ transform: `rotate(${angle}deg)`, boxShadow: '0 0 80px oklch(0.78 0.16 85 / 60%), inset 0 0 40px oklch(0 0 0 / 60%)' }}
          >
            <svg viewBox="-100 -100 200 200" className="absolute inset-0 w-full h-full">
              {SLOTS.map((slot, i) => {
                ...
                const fill = slot.kind === 'lost' ? 'oklch(0.20 0.03 15)' : accentColors[i % accentColors.length];
                return (
                  <path key={i} d={`M0 0 L${x1} ${y1} A100 100 0 0 1 ${x2} ${y2} Z`}
                    fill={fill} stroke="oklch(0.10 0 0)" strokeWidth={0.8} />
                );
              })}
            </svg>
            {SLOTS.map((slot, i) => {
              const a = i * segAngle + segAngle / 2;
              const radius = SLOTS.length > 8 ? 82 : 78;
              const size = SLOTS.length > 8 ? 38 : 56;
              const half = size / 2;
                  ...
                  {slot.kind === 'win' ? (
                    <>
                      <img src={slot.token.logo} alt={slot.token.symbol}
                        className={`${SLOTS.length > 8 ? 'w-5 h-5' : 'w-8 h-8'} rounded-full ring-2 ring-white/40`} />
            ...
            <div className="absolute inset-[40%] rounded-full bg-gradient-to-br from-wolf-gold to-wolf-red flex items-center justify-center text-2xl">🐺</div>
          </div>
        </div>
        <PendingOverlay active={tx.status === 'pending' || (tx.status === 'confirmed' && !result)} label="Lucky wheel spinning…" />
      </div>

      {/* Active Board panel — 4 rewards + 4 LOST, with realized status after confirm */}
      <div className="mt-4">
                ...
                className={`p-2 rounded-lg border-2 text-center transition-all relative ${
                  isLanded
                    ...
                    : isPicked
                      ...
                      : 'border-wolf-border/30 hover:border-wolf-gold/40'
                } ${slot.kind === 'lost' ? 'opacity-90' : ''}`}
              >
                {isLanded && (
                  <span className={`absolute -top-1.5 -right-1.5 text-[9px] font-black px-1.5 py-0.5 rounded-full ${result?.win ? 'bg-wolf-green text-black' : 'bg-wolf-red text-white'}`}>
                    {result?.win ? 'WIN' : 'LOST'}
                  </span>
                )}
                {slot.kind === 'win' ? (
                  <>
                    <img src={slot.token.logo} alt={slot.token.symbol} className="w-7 h-7 mx-auto rounded-full" />
                    <div className="text-[10px] font-semibold mt-1">{slot.token.symbol}</div>
                    <div className="text-[9px] text-wolf-gold font-mono">
                      {slot.reward < 0.001 ? slot.reward.toExponential(0) : slot.reward}
                    </div>
                  </>
                ) : (
                  <>
                    <div className="w-7 h-7 mx-auto rounded-full bg-wolf-red/20 border border-wolf-red/40 flex items-center justify-center text-xs">💀</div>
                    <div className="text-[10px] font-bold mt-1 text-wolf-red">LOST</div>
                    <div className="text-[9px] text-muted-foreground">no win</div>
                  </>
                )}
              </button>
            );
      ...
      <div className="mt-2 text-[11px] text-center text-muted-foreground">
        Picked slot: <span className={pickedSlot.kind === 'lost' ? 'text-wolf-red font-bold' : 'text-wolf-gold font-bold'}>
          {pickedSlot.kind === 'win' ? rewardLabelFor(pickedSlot.token.symbol) : 'LOST'}
        </span>
      </div>
      ...
      <PlayButton onClick={play} busy={!!casino.busy} label={betError ?? `Spin ${applyMultiplier(bet, mult)} zkLTC`} disabled={!casino.stats.isActive || !!betError} />
